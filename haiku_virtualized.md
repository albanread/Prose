...
- ✅ **app_server's back buffer in the pool (patch 0018):** new optional accelerant hook `B_GET_BACK_BUFFER_CONFIG`; app_server draws into pool memory instead of a `MallocBuffer`, so the host's GPU can reach both `bb` and `fb`. Verified with hand and scripted resizes.
- Remaining for S2 (spec §12.5–6): tell the host where `bb` is and blit bb → dispbuf on the GPU on the ready signal (dropping `fb` from the normal path); the 19 ms commit-latency outliers; cursor as a host overlay; `hvgpu` defaulting to `--display s2`; a Haiku-side pixel test.
- ✅ **MIDI to the host (2026-09-18, patch 0023).** Haiku's Midi Kit routes between producers and consumers through `midi_server`, which turns every file under `/dev/midi` into a port pair, and its old-kit soft synth is fluidsynth + a SoundFont — exactly the part the host does better. The Prose MIDI device (`tools/hvgpu/midi.swift`, virtio ID 62, one queue per direction) appears as `/dev/midi/prose/0`; what the guest plays goes to Core Audio's General MIDI (DLS) synth and to a CoreMIDI virtual source "Haiku (hvgpu)", and a CoreMIDI virtual destination of the same name feeds Mac keyboards/sequencers into the guest (host → guest not yet exercised). `prose_midi_test [--direct]` plays a scale through the Midi Kit or straight into the port; `midiprobe.sh` runs both at boot. Lessons: custom virtio IDs must stay below 64 (PCI ID 0x1040+ID must stay inside the modern range `virtio_pci` accepts); the device manager searches a virtio child's driver only at registration, in the context of the `/dev/<class>` scan that triggered it, so the module lives under `drivers/audio/hmulti` next to `virtio_sound`.
- ✅ **Sound (2026-09-18, patch 0022).** Haiku's audio is a software mixing graph (apps → `media_server`'s mixer → the `hmulti_audio` node → a Multi Audio kernel driver); the host only ever sees one PCM stream per direction. The fork's `virtio_sound` driver (stereo S16 48 kHz playback; capture is the same shape, later) turns the mixer's buffers into virtio-snd txq buffers and paces the node on a steady schedule anchored to the device's completions — hosts complete in bursts (QEMU's 10 ms audio timer) and the node drops a mixer buffer that lands while the previous one is unconsumed, which put a silent buffer every few periods until the pacing went in. Two more fixes were needed underneath: the device manager never published `/dev/audio` after boot (an upstream TODO — class directories only came from legacy drivers), and virtio interrupts now request a reschedule. The image gained the media stack (libmedia, media servers, mixer, hmulti_audio) and `prose_tone`, a `BSoundPlayer` sine generator. Host: VZ's virtio-snd to the Mac's default devices (`--no-sound` to drop it), QEMU `--sound wav|coreaudio` (`wav` records to `work/qemu/out.wav` at 48 kHz; `private_workspace/wavcheck.py` measures level and pitch). Verified: a clean 440 Hz tone on both.
- ✅ **Networking (2026-09-18, patch 0021).** It had never worked under VZ: DHCP got no reply. Two causes: Haiku's `virtio_net` used the 10-byte legacy header on a `VIRTIO_F_VERSION_1` device (12-byte `virtio_net_hdr_v1`), misframing both directions; and Apple's virtio-net refuses `FEATURES_OK` for any set without `VIRTIO_NET_F_CSUM`, which the driver ignored and ran with no features (no MAC, device in FAILED state). Now: header size follows the negotiated features, the driver asks for CSUM and checks the result, the bus manager logs rejections and retries (everything offered → bare transport), and the MTU is a frame size again (1500, not 1486). `private_workspace/netprobe.sh` (injected as the guest's UserBootscript by `PW_INJECT_SCRIPT=`) plus `extract.sh` verify it unattended: lease in the first second, gateway 0.2 ms, 1.1.1.1 ~29 ms, DNS, 1400-byte pings.

**Sprint 3: polish and reach.**
- ✅ **Our own virtio-input keyboard and tablet (2026-09-18, `tools/hvgpu/input.swift`, default `--input own`).** Two more custom virtio devices (ID 18), fed from our Metal view's NSEvents; pointer coordinates go through the presenter's aspect-fit rectangle, so they are exact at any window size. VZ's virtio-gpu, USB keyboard, pointing device and `VZVirtualMachineView` are gone from the VM (`--input vz` brings the old stack back); patch 0002 no longer matters on VZ, and boot is ~1 s faster without VZ's GPU to probe. VZ delivers no configuration writes to custom devices, which virtio-input's select/subsel protocol needs, so the device publishes a static answer table after the standard configuration (marked `PRIN1`) and the fork's `virtio_input` driver reads that instead (patch 0019); on QEMU the same driver still speaks the standard protocol. Key mapping is positional (ANSI), ⌘ → Haiku Command, ⌥ → Haiku Option; the guest does key repeat; the host cursor hides over the window. `hvgpu --input-test` clicks the Deskbar leaf, presses Escape and parks the pointer, with a screenshot after each step — all three verified.
- ✅ **HostFS: a Mac folder as a disk (2026-09-18, patches 0024–0025, `tools/hvgpu/hostfs.swift`).** VZ's own virtio-fs device (PCI 1AF4:105A, class mass storage/other) with Apple's FUSE server behind it does the host side: `hvgpu --share DIR` (`--share-ro`, `--share-tag`; several shares become one volume with a folder each), and `run-vz.sh` shares `~/Documents/HostFS` (a folder of its own, not the home folder) unless `PW_SHARE` says otherwise. Guest: `virtio_fs` (a boot module — mass-storage PCI devices are only probed in the `/dev/disk` scan at boot) moves FUSE requests through 8 DMA slots and publishes the device as a 64 KiB read-only pseudo-disk naming its tag; the `hostfs` file system (FUSE 7.31 client) recognises it, so mount_server mounts it at boot at `/HostFS` like any disk (patched to always mount HostFS) and it is on the Desktop, in the Mount menu and DriveSetup. Node IDs are vnode IDs with FORGET bookkeeping (idle nodes kept up to 2048); files go through the file cache with close-to-open consistency, so mmap and executables work; Haiku attributes are host xattrs `user.haiku.<name>` (the build tools' convention, so `xattr` on the Mac shows them), `BEOS:TYPE` comes from the extension when none is stored; directories are read with READDIRPLUS so `d_ino` matches vnode IDs. Apple's server: FUSE 7.31, max_write 128 KiB, grants async read, atomic O_TRUNC, big writes, readdirplus, parallel dirops; no export support, no notification queue (host-side changes are seen on open/stat, not pushed). Measured: 1.5 GB/s uncached read, 1.6 GB/s write including the flush at close, 2000 small files created in 0.46 s, `ls -l` of 1000 host files 80 ms. `private_workspace/hostfs-test.sh` checks it end to end. Not on QEMU (no virtio-fs server on macOS QEMU).
