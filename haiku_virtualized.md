...
- Remaining for S2 (spec §12.5–6): tell the host where `bb` is and blit bb → dispbuf on the GPU on the ready signal (dropping `fb` from the normal path); the 19 ms commit-latency outliers; cursor as a host overlay; a Haiku-side pixel test.
