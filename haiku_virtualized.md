...
- Next, milestone C: live resize (`MODE_HINT` → app_server mode change), the presenter sampling the pool directly (drop the host-side copy), `REDRAW` handling, then retire S1 as the default.
