#!/bin/sh
# probe for boot-test.sh: packages/boot-test.sh packages/tests/openssl.sh openssl3
...
# exercise the SHA-512/SHA-3 hardware paths.
echo "== $(uname -a)"
openssl version || { echo FAIL; exit; }
ok=1
check() {
	got=$(printf abc | openssl dgst -"$1" | sed 's/.*= //')
...
for alg in aes-128-gcm sha256 sha512; do
	fast=$(openssl speed -elapsed -seconds 1 -bytes 16384 -evp $alg 2>/dev/null | tail -1 | awk '{print $NF}')
	slow=$(OPENSSL_armcap=0 openssl speed -elapsed -seconds 1 -bytes 16384 -evp $alg 2>/dev/null | tail -1 | awk '{print $NF}')
