...
- **The back buffer in the pool (patch 0018).** The driver reserves a region after the space for the largest front buffer (`bb_offset` = 16 KiB-aligned `stride_align(max_width × 4) × max_height`, 0x1fa4000 for 3840x2160; needs a pool ≥ 2× that, hence the 128 MiB default) and publishes it in the shared info. The fork adds the optional accelerant hook `B_GET_BACK_BUFFER_CONFIG` (0x501, `get_back_buffer_config(frame_buffer_config*)`); app_server asks for it whenever it (re)creates its back buffer and, when it succeeds, draws into that memory (`DeviceBackBuffer`, always B_RGBA32, the mode's size, the front buffer's stride) instead of a `MallocBuffer`. Both buffers app_server works with are now in the pool: the host's GPU can reach `bb` and `fb`, which is what a bb → dispbuf blit kernel needs. app_server's memset after creation takes the first-touch cost (~1.2 ms per 4 MiB). Verified: desktop rendering and hand/scripted resizes re-create it at each new size.
- Live resize (patch 0016): `MODE_HINT` updates `pref_*` in the shared info and wakes `PRDS_WAIT_MODE_HINT`, an ioctl that blocks (with timeout, interruptible) inside the driver — so the semaphore behind it stays kernel-owned. `prose_display_agent`, a small server launched from `data/launch/system`, waits on it and switches the screen with `BScreen::SetMode()` (timing from `compute_display_timing()`, any size up to `max_*`). A drag produces a hint per mouse move: the agent switches once, 250 ms after the last one (capped at 1.5 s for long drags), and the ioctl returns immediately when the host's size differs from the one last applied so a hint can never be lost (patch 0020; `hvgpu --resize-drag N WxH` simulates the drag: 40 hints → 1 switch); the accelerant accepts the size, the driver sends `SET_MODE`, app_server re-reads the frame buffer config and repaints. Measured: host window resize → guest desktop at the new size in ~160 ms. `REDRAW` bumps `redraw_count` in the shared info and the accelerant's commit thread answers with a full-surface commit (the host does not send `REDRAW` yet).
...
5. Commit round-trip outliers: 50 µs average but a 19 ms maximum every few thousand commits. Candidates: `virtio_pci`'s interrupt handler returns `B_HANDLED_INTERRUPT` even when a queue callback woke a thread (`B_INVOKE_SCHEDULER` would reschedule at once), or host-side scheduling of the device queue. Worth fixing before a GPU blit path makes commits longer. *Measured 2026-09-18, after `virtio_pci` returned `B_INVOKE_SCHEDULER` and patch 0030 made the bus manager re-check a queue after turning its interrupt back on: four idle-desktop runs of 10000 commits, average 49–60 µs; the maxima (0.6, 1.4, 2.0 and 12.8 ms) all fell in the first 2000 commits, during desktop start, and none in steady state. The re-check never fired on the display's queues in those runs; under parallel HostFS load it caught 64+ completions on virtio-fs.*
6. Next steps toward the ideal `dispbuf ← fb ← bb` with no CPU copies: ✅ app_server's back buffer is in the pool (patch 0018); next, tell the host where it is (`SET_MODE` v2 or a `SET_BACK_BUFFER` command carrying `bb_offset`/stride) and let a host blit kernel copy bb → dispbuf on the ready signal, dropping the fb from the normal path (it stays for BDirectWindow clients, which draw into the front buffer on their own schedule), and move the cursor to a host overlay through the `B_SET_CURSOR_SHAPE`/`B_MOVE_CURSOR` hooks so cursor moves no longer touch the front buffer at all. Pointer swapping (flipping the three buffers) does not fit app_server's damage model — a flipped-in buffer lacks the previous frames' damage and would need catch-up copies of the same size — so the win is in *who* copies (the GPU), not in avoiding the copy.
