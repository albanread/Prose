...
- The presenter samples the pool directly: the same `mmap`ed memory is mapped into the guest by VZ and wrapped for Metal with `makeBuffer(bytesNoCopy:)` (page aligned, `storageModeShared`), and the fragment shader indexes it with the mode's stride and pool offset. Nothing is copied on the host, ever. `COMMIT` therefore only validates and counts the rects and marks the surface changed; the next display-link tick presents. (Milestone A copied committed rects into a private surface; that path is gone.) Consequence: the "host is done reading" meaning of completion is vacuous — the host may read the pool at any time, like any scanout — which v1 guests never depend on. "Black until the first commit after `SET_MODE`" holds: the presenter clears to black while no commit has arrived for the current mode.
...
- Accelerant `prose_display.accelerant`: the frame buffer app_server draws into *is* the pool (zero copy). Mode list from `create_display_modes()`: the host's preferred size plus the standard modes that fit `max_*`, B_RGB32 only; `B_GET_PREFERRED_DISPLAY_MODE` follows the host window (`pref_*`, updated by `MODE_HINT`); `B_GET_FRAME_BUFFER_CONFIG` = pool + `mode_offset`, `bytes_per_row` = stride; `B_ACCELERANT_RETRACE_SEMAPHORE` = the driver's semaphore. The commit hook collects rectangles under a lock (collapsing to one bounding box beyond 64) and a commit thread sends one `PRDS_COMMIT` per vsync — it waits on the retrace semaphore with a 20 ms timeout, so the desktop keeps updating without vsync (headless host, display asleep, or a semaphore it may not acquire).
- app_server (fork): `Accelerant.h` gains the optional hook `B_COMMIT_RECTANGLES` (0x500), `status_t commit_rectangles(const fill_rect_params* list, uint32 count)`. `AccelerantHWInterface` calls it after `_CopyBackToFront(region)` with the region's rectangles (32 per call) and from `_DrawCursor()`, because the software cursor is drawn into the front buffer past the back buffer. Accelerants without the hook are unaffected. Measured: boot drawing coalesces to at most one commit per vsync; an idle desktop commits ~4 times/s.
